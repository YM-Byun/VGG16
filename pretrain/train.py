...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader):
        ...
        if (i % 20 == 19) or (i == len(train_loader) - 1):
            ...
            print (f"Epoch [{epoch+1}/{epochs}] | Train iter [{i+1}/{len(train_loader)}] | acc = {acc[0].val:.5f} | loss = {(running_loss / float(i+1)):.5f}")
...
def validate(val_loader, model, criterion, epoch):
    ...
    with torch.no_grad():
        ...
        if (i % 10 == 9) or (i == len(train_loader) - 1):
            print (f"Epoch [{epoch+1}/{epochs}] | Val iter [{i+1}/{len(train_loader)}] | acc = {acc[0].val:.5f} | loss = {(running_loss / float(i)):.5f}")
